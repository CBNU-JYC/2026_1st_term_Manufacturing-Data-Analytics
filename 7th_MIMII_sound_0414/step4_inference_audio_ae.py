...
# %% [2] 추론용 비정상 오디오 파일 선택
abnormal_files = glob.glob(os.path.join(BASE_DIR, '0_dB_valve', 'valve', 'id_02', 'abnormal', '*.wav'))
...
plt.legend()
save_current_figure("step4_inference_scores.png")
plt.show()

results_df = pd.DataFrame({
    "time_sec": times,
    "anomaly_score": scores,
    "threshold": [THRESHOLD] * len(times),
    "predicted_status": ["ABNORMAL" if score > THRESHOLD else "NORMAL" for score in scores],
})
results_csv_path = os.path.join(RESULTS_DIR, "step4_inference_scores.csv")
