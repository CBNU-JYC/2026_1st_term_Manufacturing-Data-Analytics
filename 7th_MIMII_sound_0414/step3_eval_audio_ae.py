...
model.load_state_dict(torch.load(os.path.join(BASE_DIR, 'audio_models', 'audio_autoencoder_real.pth'), map_location=device))
model.eval()
print("실제 밸브 학습 모델 로드 완료")

# %% [2] 평가용 데이터 경로 설정 (정상/비정상)
normal_test_dir = os.path.join(BASE_DIR, '0_dB_valve', 'valve', 'id_02', 'normal')
abnormal_test_dir = os.path.join(BASE_DIR, '0_dB_valve', 'valve', 'id_02', 'abnormal')
...
plt.legend()
save_current_figure("step3_error_distribution.png")
plt.show()
...
plt.legend()
save_current_figure("step3_roc_curve.png")
plt.show()
...
).to_csv(os.path.join(RESULTS_DIR, "step3_threshold_search.csv"), index=False)
...
save_current_figure("step3_confusion_matrix.png")
plt.show()
with open(os.path.join(RESULTS_DIR, "step3_eval_metrics.json"), "w", encoding="utf-8") as f:
    json.dump(
        {
...
).to_csv(os.path.join(RESULTS_DIR, "step3_predictions.csv"), index=False)
...
def visualize_reconstruction(file_path, title):
    ...
    save_current_figure(f"step3_reconstruction_{safe_title}.png")
    plt.show()
