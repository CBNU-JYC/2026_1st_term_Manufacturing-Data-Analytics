...
### OPC-UA 기본 클라이언트 실행 예시

```text
서버에 연결 중: opc.tcp://127.0.0.1:4840/freeopcua/server/ ...
연결 성공!
현재 온도 초기값: 30.0
온도 데이터 구독을 시작합니다. (Ctrl+C로 종료)
[데이터 수신] 노드: ns=2;i=2, 변경된 온도 값: 30.0
[데이터 수신] 노드: ns=2;i=2, 변경된 온도 값: 30.5
[데이터 수신] 노드: ns=2;i=2, 변경된 온도 값: 31.0
...
구독 종료 및 클라이언트 연결 해제
```
...
- `opc_client_mfg.py` 실행 시 온도와 압력 값을 일정 주기로 수집합니다.
- 수집 결과는 `manufacturing_sensor_data.csv`로 저장됩니다.
- 데이터 파이프라인 실행 시 정제 전/후 CSV와 최종 데이터셋이 생성됩니다.
